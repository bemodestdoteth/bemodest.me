...
pub use generated::*;
pub use generated::{
    AlertRuleCondition as Condition, NormalizedTickerExchange as Exchange,
    NormalizedTickerMarketState as MarketState, SidecarConfigPayloadType as Type,
};
    ...
    for (prefix, divisor) in PREFIXES {
        if let Some(stripped) = base.strip_prefix(prefix) {
            if !stripped.is_empty() {
                return (stripped.to_string(), rust_decimal::Decimal::from(*divisor));
            }
        }
    }
