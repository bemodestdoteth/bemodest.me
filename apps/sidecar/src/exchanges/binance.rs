...
impl BinanceExchange {
    ...
    async fn connect_and_loop(
        ...
        loop {
            if verbose {
                info!("Connecting to Binance WebSocket: {}", TICKER_STREAM_URL);
            }
            
            match connect_async(TICKER_STREAM_URL).await {
                Ok((ws_stream, _)) => {
                    if verbose {
                        info!("Binance Spot WebSocket connected.");
                    }
                    ...
                    let mut batcher = TickerBatcher::new(tx.clone(), "binance".to_string(), lvc.clone(), filter);
                                ...
                                match msg_res {
                                    Ok(Message::Text(text)) => {
                                        if let Ok(raw) = serde_json::from_str::<Value>(&text) {
                                            let usdt_only: Value = match raw.as_array() {
                                            ...
                                            for mut ticker in normalized {
                                                ...
                                                trace!("[Binance] Normalized: {}/{} c={}", ticker.base, ticker.quote, ticker.c);
                                    ...
                                    Ok(Message::Ping(payload)) => {
                                        if let Err(e) = write.send(Message::Pong(payload)).await {
                                            error!("Failed to send pong to Binance: {}", e);
                                            break;
                                        } else {
                                            debug!("Sent pong to Binance");
                                        }
                                    }
                                    Ok(Message::Close(_)) => {
                                        if verbose {
                                            info!("Binance Spot WebSocket connection closed. Reconnecting in {}s...", RECONNECT_DELAY_SECONDS);
                                        }
                                        break;
                                    }
                                    Err(e) => {
                                        if verbose {
                                            error!("An error occurred with Binance Spot WebSocket: {}. Reconnecting in {}s...", e, RECONNECT_DELAY_SECONDS);
                                        }
                                        break;
                                    }
                                    _ => {}
                ...
                Err(e) => {
                    ...
                    if verbose {
                        error!("Failed to connect to Binance WebSocket: {}. Reconnecting in {}s...", e, RECONNECT_DELAY_SECONDS);
                    }
                }
            }
...
impl Exchange for BinanceExchange {
    async fn connect(&mut self) {
        ...
        tokio::spawn(async move {
            info!("[BinanceExchange] Connection task started");
