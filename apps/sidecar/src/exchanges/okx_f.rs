...
/// OKX Public WebSocket endpoint for futures (Swap) tickers
...
/// Send a ping string every 25 seconds
...
impl OkxFExchange {
    ...
    async fn run_shard(
        shard_idx: usize,
        total_shards: usize,
        symbols_chunk: Vec<String>, // these are okx f instIds e.g. BTC-USDT-SWAP
        tx: broadcast::Sender<String>,
        connected: Arc<AtomicBool>,
        ...
        loop {
            if verbose {
                info!("[OkxFExchange] [Shard {}] Connecting ({} symbols)", shard_idx, symbols_chunk.len());
            }

            let ws_url_parsed = match WS_URL.parse::<tokio_tungstenite::tungstenite::http::Uri>() {
                Ok(u) => u,
                Err(e) => {
                    error!("[OkxFExchange] [Shard {}] Invalid WS URL: {}", shard_idx, e);
            ...
            match connect_async(ws_url_parsed).await {
                Ok((ws_stream, _)) => {
                    ...
                    for (batch_idx, chunk) in symbols_chunk.chunks(SUBSCRIBE_BATCH_SIZE).enumerate() {
                        ...
                        if let Err(e) = write.send(Message::Text(sub_msg.to_string().into())).await {
                            error!("[OkxFExchange] [Shard {}] Failed to send sub batch {}: {}", shard_idx, batch_idx, e);
                            sub_ok = false;
                            break;
                        }
                    ...
                    if verbose {
                        info!("[OkxFExchange] [Shard {}] Subscribed to {} tickers", shard_idx, symbols_chunk.len());
                    }
                        ...
                        tokio::select! {
                            _ = ping_interval.tick() => {
                                ...
                                if let Err(e) = write.send(Message::Text("ping".into())).await {
                                    error!("[OkxFExchange] [Shard {}] Failed to send ping: {}", shard_idx, e);
                                    break;
                                } else {
                                    trace!("[OkxFExchange] [Shard {}] Sent ping", shard_idx);
                                }
                            }
                            ...
                            msg_res = read.next() => {
                                ...
                                match msg_res {
                                    Ok(Message::Text(text)) => {
                                        // OKX ping response
                                        if text.trim() == "pong" {
                                            trace!("[OkxFExchange] [Shard {}] Received pong", shard_idx);
                                            continue;
                                        }
                                        ...
                                        if let Ok(raw) = serde_json::from_str::<Value>(&text) {
                                            ...
                                            if let Some(event) = raw.get("event").and_then(|v| v.as_str()) {
                                                match event {
                                                    "subscribe" => debug!("[OkxFExchange] [Shard {}] Subscribe ack", shard_idx),
                                                    "error" => {
                                                        let code = raw.get("code").and_then(|v| v.as_str()).unwrap_or("?");
                                                        let msg  = raw.get("msg").and_then(|v| v.as_str()).unwrap_or("?");
                                                        error!("[OkxFExchange] [Shard {}] Server error {}: {}", shard_idx, code, msg);
                                                    }
                                                    _ => debug!("[OkxFExchange] [Shard {}] Unhandled event '{}'", shard_idx, event),
                                                }
                                                continue;
                                            }
                                    ...
                                    Ok(Message::Ping(payload)) => {
                                        if let Err(e) = write.send(Message::Pong(payload)).await {
                                            error!("[OkxFExchange] [Shard {}] Failed to send pong: {}", shard_idx, e);
                                            break;
                                        }
                                    }
                                    Ok(Message::Close(_)) => {
                                        if verbose {
                                            info!("[OkxFExchange] [Shard {}] Connection closed by server.", shard_idx);
                                        }
                                        break;
                                    }
                                    Err(e) => {
                                        if verbose {
                                            error!("[OkxFExchange] [Shard {}] WebSocket error: {}", shard_idx, e);
                                        }
                                        break;
                                    }
                                    _ => {}
                ...
                Err(e) => {
                    ...
                    let _ = tx.send(status.to_string());
                    if verbose {
                        error!("[OkxFExchange] [Shard {}] Failed to connect: {}. Reconnecting in {}s…", shard_idx, e, RECONNECT_DELAY_SECONDS);
                    }
                }
            }
    ...
    async fn manager_loop(
        ...
        let symbols = market_cache.get_okx_f_markets().await;
        if symbols.is_empty() {
            warn!("[OkxFExchange] No symbols available to subscribe to.");
            return;
        }
        ...
        info!("[OkxFExchange] Distributed {} symbols across {} shards", symbols.len(), total_shards);
    }
}
...
impl Exchange for OkxFExchange {
    async fn connect(&mut self) {
        ...
        tokio::spawn(async move {
            info!("[OkxFExchange] Manager task started");
