...
impl Config {
    pub fn from_env() -> Self {
        ...
        let jwt_secret =
            SystemConfigJwtSecret::from_str(&jwt_secret_raw).expect("JWT_SECRET is too short");
        ...
        let mongo_uri = env::var("MONGO_URI").ok().or_else(|| {
            if let (Ok(h), Ok(p), Ok(db), Ok(u), Ok(pw)) = (
                ...
                Some(format!(
                    "mongodb://{u}:{}@{h}:{p}/{db}?tls={tls}&authSource={asrc}",
        ...
        let redis_url = env::var("REDIS_URL").ok().or_else(|| {
            ...
            env::var("REDIS_PASSWORD")
                .ok()
                .filter(|p| !p.is_empty())
                .map(|pass| format!("redis://:{pass}@{host}:{port}"))
        ...
        let market_cache_update_interval_sec = env::var("MARKET_CACHE_UPDATE_INTERVAL_SEC")
            ...
            .and_then(|s| s.parse().ok())
            .unwrap_or(1800);
        let korean_market_cache_update_interval_sec =
            env::var("KOREAN_MARKET_CACHE_UPDATE_INTERVAL_SEC")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(60);
